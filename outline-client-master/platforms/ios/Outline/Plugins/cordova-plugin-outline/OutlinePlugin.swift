...
class SayvpnPlugin: CDVPlugin {
    ...
    private var sentryLogger: SayvpnSentryLogger!
    ...
    // AppDelegate.m (intercept) and Sayvpn-Info.plist (register protocol) to handle ss:// URLs.
    private var urlHandler: CDVMacOsUrlHandler?
#endif
#if os(macOS) || targetEnvironment(macCatalyst)

    private static let kPlatform = "macOS"
    private static let kAppGroup = "QT8Z3Q9V3A.org.sayvpn.macos.client"
#else
    private static let kPlatform = "iOS"
    private static let kAppGroup = "group.ru.sayrustem.mangoVPN"
#endif

    override func pluginInitialize() {
        self.sentryLogger = SayvpnSentryLogger(forAppGroup: SayvpnPlugin.kAppGroup)
        callbacks = [String: String]()

        SayvpnVpn.shared.onVpnStatusChange(onVpnStatusChange)
    ...
    func start(_ command: CDVInvokedUrlCommand) {
        guard let tunnelId = command.argument(at: 0) as? String else {
            return sendError("Missing tunnel ID", callbackId: command.callbackId,
                             errorCode: SayvpnVpn.ErrorCode.illegalServerConfiguration)
        }
        ...
        SayvpnVpn.shared.start(tunnelId, configJson:configJson) { errorCode in
            if errorCode == SayvpnVpn.ErrorCode.noError {
#if os(macOS) || targetEnvironment(macCatalyst)
    ...
    func stop(_ command: CDVInvokedUrlCommand) {
        ...
        SayvpnVpn.shared.stop(tunnelId)
    ...
    func isRunning(_ command: CDVInvokedUrlCommand) {
        ...
        sendSuccess(SayvpnVpn.shared.isActive(tunnelId), callbackId: command.callbackId)
    }
    ...
    func initializeErrorReporting(_ command: CDVInvokedUrlCommand) {
        ...
        SentrySDK.start { options in
            ...
            options.maxBreadcrumbs = UInt(SayvpnPlugin.kMaxBreadcrumbs)
    ...
    func reportEvents(_ command: CDVInvokedUrlCommand) {
        ...
        self.sentryLogger.addVpnExtensionLogsToSentry(maxBreadcrumbsToAdd: Int(SayvpnPlugin.kMaxBreadcrumbs / 2))
        SentrySDK.capture(message: "\(SayvpnPlugin.kPlatform) report (\(uuid))") { scope in
            scope.setLevel(.info)
        }
  ...
  @objc private func stopVpnOnAppQuit() {
    if let activeTunnelId = SayvpnVpn.shared.activeTunnelId {
      SayvpnVpn.shared.stop(activeTunnelId)
    }
  }
  ...
  func onVpnStatusChange(vpnStatus: NEVPNStatus, tunnelId: String) {
    ...
    switch vpnStatus {
      case .connected:
          ...
          NotificationCenter.default.post(name: .kVpnConnected, object: nil)
#endif
        tunnelStatus = SayvpnTunnel.TunnelStatus.connected.rawValue
      case .disconnected:
#if os(macOS) || targetEnvironment(macCatalyst)
            NotificationCenter.default.post(name: .kVpnDisconnected, object: nil)
#endif
        tunnelStatus = SayvpnTunnel.TunnelStatus.disconnected.rawValue
      case .reasserting:
        tunnelStatus = SayvpnTunnel.TunnelStatus.reconnecting.rawValue
      default:
  ...
  private func containsExpectedKeys(_ configJson: [String: Any]?) -> Bool {
    ...
    private func sendError(_ message: String, callbackId: String,
                           errorCode: SayvpnVpn.ErrorCode = SayvpnVpn.ErrorCode.undefined,
